 ...
 *
 * The max prime number which is able to obtain is 8161, and its corresponding index is 1023.
 * $(BR)If you want more bigger, don't use this function!
 *
 * Params:
 *   beg= index of the first prime number.
 *   len= total prime numbers you require.
 *   step= step of the series of prime numbers.
 *
 * Returns:
 *   prime numbers as a dynamic array
 *************************************************************/
T[] getPrimeNumbers(T)(in size_t beg, in size_t len, in size_t step= 1u) @safe pure nothrow
if(((isIntegral!T && T.max >= ushort.max) || isFloatingPoint!T))
in(len > 0u && len <= 1024 && step > 0u){
	import std.range: iota, zip;

	typeof(return) result= new T[len];

	foreach(idxDest, idxSrc; zip(iota(0, len, 1),
	                             iota(beg, beg+len*step, step))
	        ) result[idxDest]= cast(T)(PRIME_NUMBER[idxSrc]);
...
unittest{
	...
	{
		double[] num= getPrimeNumbers!double(0, 25, 2);
	...
	{
		int [] num= getPrimeNumbers!int(240, 16, 3);
...
package:
// prime numbers
immutable ushort[1024] PRIME_NUMBER= [
	...
	0x062B, 0x062F, 0x063D, 0x0641, 0x0647, 0x0649, 0x064D, 0x0653,	// 191-255
	...
	0x0E1D, 0x0E21, 0x0E27, 0x0E2F, 0x0E35, 0x0E3B, 0x0E4B, 0x0E57,	// 448-511
	...
	0x1025, 0x102B, 0x1039, 0x103D, 0x103F, 0x1051, 0x1069, 0x1073,	// 512-575
	...
	0x1247, 0x1253, 0x125F, 0x1271, 0x1273, 0x1279, 0x127D, 0x128F,	// 576-639
	...
	0x1475, 0x148D, 0x1499, 0x149F, 0x14A1, 0x14B1, 0x14B7, 0x14BD,	// 640-703
	...
	0x16A9, 0x16AF, 0x16B5, 0x16BD, 0x16C3, 0x16CF, 0x16D3, 0x16D9,	// 704-767
	...
	0x18C1, 0x18C7, 0x18D1, 0x18D7, 0x18D9, 0x18DF, 0x18E5, 0x18EB,	// 768-831
	...
	0x1AFF, 0x1B05, 0x1B23, 0x1B25, 0x1B2F, 0x1B31, 0x1B37, 0x1B3B,	// 832-895
	...
	0x1D63, 0x1D69, 0x1D71, 0x1D75, 0x1D7B, 0x1D7D, 0x1D87, 0x1D89,	// 896-959
	...
	0x1F99, 0x1F9D, 0x1FA5, 0x1FAF, 0x1FB5, 0x1FBB, 0x1FD3, 0x1FE1];	// 960-1023
